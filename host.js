...
async function setupBuzzerRoom() {
    ...
    // Vi lägger till event: null här så databasen vet att inget event pågår
...
function initiateGameLogic() {
    viewedQuestions = {}; 
    questionsOpened = 0; 
    dailyDoubleTriggered = false; 
    allaSvararCount = 0; 
    raddningsplankanTriggered = false; 
    frozenTeam = null; 
    nextFrozenTeam = null; 
    mappedEvents = {};
    ...
    if(Math.random() < 0.33) { 
        do { 
            solidaritetTurn = Math.floor(Math.random() * 20) + 4; 
        } while (assignedTurns.includes(solidaritetTurn)); 
    } else { 
        solidaritetTurn = -1; 
    }
...
function setupGameBoard() {
    ...
    currentBoard.categories.forEach(cat => {
        ...
        catDiv.textContent = cat || 'Kategori'; 
        catDiv.classList.add('category-cell');
    ...
    for (let row = 0; row < 5; row++) {
        for (let col = 0; col < 6; col++) {
            ...
            qDiv.classList.add('question-cell'); 
            qDiv.dataset.col = col; 
            qDiv.dataset.row = row;
...
function updateTeamsDisplay() {
    ...
    teams.forEach(team => {
        ...
        nameInput.onchange = async (e) => {
            ...
            if(!newName || teams.includes(newName)) { 
                e.target.value = team; 
                return; 
            }
            teamScores[newName] = teamScores[team]; 
            delete teamScores[team];
            ...
            Object.entries(playersMap).forEach(async ([uid, pData]) => {
                if(pData.team === team) {
                    await updateDoc(doc(db, "rooms", roomId, "players", uid), { team: newName });
                }
            });
            updateTeamsDisplay();
        };
        ...
        scoreP.textContent = `${teamScores[team]} p`;
        ...
        playersDiv.addEventListener('dragleave', e => { 
        ...
        playersDiv.addEventListener('drop', async e => {
            ...
            if(uid && playersMap[uid] && playersMap[uid].team !== team) {
                await updateDoc(doc(db, "rooms", roomId, "players", uid), { team: team }); 
            }
        });

        teamDiv.append(nameInput, scoreP, playersDiv); 
...
function handleQuestionClick(e) {
    ...
    viewedQuestions[`${col}-${row}`] = true; 
    cell.classList.add('clicked'); 
    cell.textContent = '';
    
    currentQuestionValue = (row + 1) * 100; 
    questionsOpened++; 
    activeDDValue = currentQuestionValue;
    currentQuestionPreEvent = null; 
    currentQuestionPostEvent = null; 
    frozenTeam = nextFrozenTeam; 
    nextFrozenTeam = null;
    ...
    if ((isDD || isRaddning || isAlla) && solidaritetTurn === questionsOpened) { 
        solidaritetTurn++; 
    }
    ...
    if (mappedEvents[questionsOpened]) {
        ...
        if (ev === 'lotterihjulet') {
            triggerLotterihjulet(col, row);
        } else if (ev === 'solospelaren') { 
            currentQuestionPreEvent = 'solospelaren'; 
            // VIKTIGT: Säg åt databasen (och eleverna) att röstning ska börja
            updateDoc(doc(db, "rooms", roomId), { event: 'solospelaren' }); 
        ...
        } else if (ev === 'dubbeltrubbel') { 
            currentQuestionPreEvent = 'dubbeltrubbel'; 
            activeDDValue *= 2; 
            flashEventSplash("DUBBELTRUBBEL!", "Värd dubbelt så mycket.<br>Gissar man fel får man dubbla minus!", "radial-gradient(circle, #8b0000, #ff0000)", "Visa Frågan", () => showQuestionPopup(col, row)); 
        } else if (ev === 'alliansen') { 
            currentQuestionPreEvent = 'alliansen'; 
            allianceTeams = getLowestTeams(2); 
            flashEventSplash("ALLIANSEN!", `${allianceTeams.join(" & ")} delar på poängen!<br>Svarar en rätt, får båda poäng!`, "radial-gradient(circle, #4169e1, #1e90ff)", "Visa Frågan", () => showQuestionPopup(col, row)); 
        } else if (ev === 'frysstralen') { 
            currentQuestionPreEvent = 'frysstralen'; 
            flashEventSplash("FRYSSTRÅLEN!", "Laget som svarar rätt får<br>frysa ett annat lag nästa runda!", "radial-gradient(circle, #00ced1, #afeeee)", "Visa Frågan", () => showQuestionPopup(col, row)); 
        } else if (ev === 'robin_hood') { 
            currentQuestionPreEvent = 'robin_hood'; 
            flashEventSplash("ROBIN HOOD!", "Rätt svar stjäl poängen från ledaren!", "radial-gradient(circle, #228b22, #32cd32)", "Visa Frågan", () => showQuestionPopup(col, row)); 
        } else if (ev === 'jackpot') { 
            currentQuestionPostEvent = 'jackpot'; 
            showQuestionPopup(col, row); 
        } 
        return;
    }

    if (solidaritetTurn === questionsOpened) { 
        currentQuestionPostEvent = 'solidaritet'; 
        showQuestionPopup(col, row); 
        return; 
    }
...
function triggerLotterihjulet(col, row) {
    const splash = document.getElementById('generic-event-splash'); 
    document.getElementById('generic-event-title').innerHTML = "LOTTERIHJULET!"; 
    ...
    let interval = setInterval(() => {
        ...
        document.getElementById('generic-event-sub').innerHTML = `Bonus: +${bonus} p`; 
        spins++;
        
        if(spins > 20) {
            clearInterval(interval); 
            newBtn.style.display = 'block'; 
            newBtn.textContent = "Visa Frågan";
            newBtn.onclick = () => { 
                splash.style.display = 'none'; 
                activeDDValue += bonus; 
                currentQuestionPreEvent = 'lotteri'; 
                showQuestionPopup(col, row); 
            };
        }
    }, 100);
...
function triggerDailyDouble(col, row) { 
    const splash = document.getElementById('daily-double-splash'); 
    splash.style.display = 'flex'; 
    
    setTimeout(() => { 
        splash.style.display = 'none'; 
        openDDSetupModal(col, row); 
    }, 2500); 
}

function triggerAllaSvarar(col, row) { 
    currentQuestionPreEvent = 'alla'; 
    flashEventSplash("ALLA SVARAR!", "Inget buzzin!<br>Gör er redo för att viska fram svaret.", "radial-gradient(circle, #008000, #20b2aa)", "Visa Frågan", () => showQuestionPopup(col, row)); 
}

function triggerRaddningsplankan(col, row) { 
    let lt = getLowestTeams(1); 
    currentQuestionPreEvent = 'raddning'; 
    allianceTeams = lt; 
    flashEventSplash("RÄDDNINGS<br>PLANKAN!", `Endast ${lt[0]} får svara först!`, "radial-gradient(circle, #800080, #ff1493)", "Visa Frågan", () => showQuestionPopup(col, row)); 
}

function openDDSetupModal(col, row) {
    const modal = document.getElementById('dd-setup-modal'); 
    const select = document.getElementById('dd-team-select'); 
    select.innerHTML = '';
    
    teams.forEach(t => { 
        const opt = document.createElement('option'); 
        opt.value = t; 
        opt.textContent = `${t} (${teamScores[t]} p)`; 
        select.appendChild(opt); 
    });
    
    const updateBetBtns = () => {
        const score = teamScores[select.value]; 
        document.getElementById('dd-bet-x2').textContent = `Ruta x 2 (${currentQuestionValue * 2} p)`;
        document.getElementById('dd-bet-all').textContent = `Alla sina poäng (${score} p)`; 
        document.getElementById('dd-bet-all').disabled = (score <= 0); 
        document.getElementById('dd-bet-all').style.opacity = score <= 0 ? '0.5' : '1';
    };
    
    select.onchange = updateBetBtns; 
    updateBetBtns(); 
    
    const btnContainer = document.getElementById('dd-bet-container'); 
    const newBtnContainer = btnContainer.cloneNode(true); 
    btnContainer.parentNode.replaceChild(newBtnContainer, btnContainer);
    
    document.getElementById('dd-bet-x2').onclick = () => { 
        activeDDValue = currentQuestionValue * 2; 
        document.getElementById('dd-setup-modal').style.display = 'none'; 
        currentQuestionPreEvent = 'dd'; 
        allianceTeams = [select.value]; 
        showQuestionPopup(col, row); 
    };
    
    document.getElementById('dd-bet-1000').onclick = () => { 
        activeDDValue = 1000; 
        document.getElementById('dd-setup-modal').style.display = 'none'; 
        currentQuestionPreEvent = 'dd'; 
        allianceTeams = [select.value]; 
    ...
    document.getElementById('dd-bet-all').onclick = () => { 
        activeDDValue = teamScores[select.value]; 
        document.getElementById('dd-setup-modal').style.display = 'none'; 
        currentQuestionPreEvent = 'dd'; 
        allianceTeams = [select.value]; 
        showQuestionPopup(col, row); 
...
function showQuestionPopup(col, row) {
    ...
    document.getElementById('question-text').textContent = currentBoard.questions[col][row] || '(Ingen fråga inlagd)';
    
    const banner = document.getElementById('event-banner'); 
    const valDisplay = document.getElementById('question-value-display'); 
    banner.style.display = 'none';
    
    if (currentQuestionPreEvent === 'dd') { 
        banner.style.display = 'inline-block'; 
        banner.style.backgroundColor = '#ff8c00'; 
        banner.textContent = `DAILY DOUBLE (Spelas av: ${allianceTeams[0]})`; 
    } else if (currentQuestionPreEvent === 'alla') { 
        banner.style.display = 'inline-block'; 
        banner.style.backgroundColor = '#20b2aa'; 
        banner.textContent = `ALLA SVARAR! (Inget buzzin)`; 
    } else if (currentQuestionPreEvent === 'raddning') { 
        banner.style.display = 'inline-block'; 
        banner.style.backgroundColor = '#9370db'; 
        banner.textContent = `RÄDDNINGSPLANKAN (Förtur: ${allianceTeams[0]})`; 
    } else if (currentQuestionPreEvent === 'alliansen') { 
        banner.style.display = 'inline-block'; 
        banner.style.backgroundColor = '#4169e1'; 
        banner.textContent = `ALLIANSEN (${allianceTeams.join(" & ")})`; 
    }
    ...
    teams.forEach(team => {
        const teamDiv = document.createElement('div'); 
        teamDiv.classList.add('adjust-team');
        
        if (team === frozenTeam) { 
            teamDiv.classList.add('team-frozen'); 
            teamDiv.title = "Fryst denna runda!"; 
        }
        
        if (currentQuestionPreEvent === 'dd') { 
            if (team === allianceTeams[0]) { 
                teamDiv.style.border = '3px solid #ff8c00'; 
                teamDiv.style.backgroundColor = '#fff3e0'; 
            } else {
                teamDiv.style.opacity = '0.4'; 
            }
        } else if (currentQuestionPreEvent === 'raddning' || currentQuestionPreEvent === 'alliansen') { 
            if (allianceTeams.includes(team)) { 
                teamDiv.style.border = '3px solid #9370db'; 
                teamDiv.style.backgroundColor = '#f3e5f5'; 
            } else {
                teamDiv.style.opacity = '0.6'; 
            }
        }
        
        const teamNameP = document.createElement('p'); 
        teamNameP.textContent = team; 
        ...
        plusBtn.onclick = () => handlePointAdjustment(team, activeDDValue, true);
        
        const minusBtn = document.createElement('button'); 
        minusBtn.textContent = 'Fel (−)'; 
        minusBtn.className = 'btn-minus'; 
        minusBtn.onclick = () => handlePointAdjustment(team, activeDDValue, false);
        
        btnGroup.append(plusBtn, minusBtn); 
        teamDiv.append(teamNameP, btnGroup); 
        adjustPointsDiv.appendChild(teamDiv);
    });
}
...
function handlePointAdjustment(team, points, isCorrect) {
    if (isCorrect) {
        ...
        if (currentQuestionPreEvent === 'frysstralen') {
            ...
            const btns = document.getElementById('freeze-team-buttons'); 
            btns.innerHTML = '';
            ...
            teams.forEach(t => {
                if(t !== team) {
                    ...
                    btn.onclick = () => { 
                        ...
                        alert(`${t} är frysta under nästa fråga!`); 
                    }; 
        ...
        if (currentQuestionPreEvent === 'alliansen' && allianceTeams.includes(team)) { 
            allianceTeams.forEach(t => { 
                if(t !== team) { teamScores[t] += points; } 
            }); 
        ...
        if (currentQuestionPreEvent === 'robin_hood') {
            if (team !== leader) { 
                ...
                flashEventSplash("STÖLD!", `${team} stal ${points}p från ${leader}!`, "radial-gradient(circle, #228b22, #006400)", "Fortsätt", ()=>{}); 
            } else { 
                flashEventSplash("SÄKERT!", `Ledande ${team} tog poängen och säkrade sin ledning!`, "radial-gradient(circle, #228b22, #006400)", "Fortsätt", ()=>{}); 
            }
        }
        ...
        if (currentQuestionPostEvent === 'jackpot') {
            ...
            if (team !== newLeader) { 
                let diff = teamScores[newLeader] - teamScores[team]; 
                let bonus = Math.floor(diff / 2); 
                teamScores[team] += bonus; 
                flashEventSplash("JACKPOT!", `Mysterieruta!<br>${team} halverar avståndet till ledaren!<br>+${bonus}p extra!`, "radial-gradient(circle, #ffd700, #ff8c00)", "Fantastiskt!", ()=>{ updateTeamsDisplay(); }); 
            }
        }
        
        if (currentQuestionPostEvent === 'solidaritet') {
            currentQuestionPostEvent = null; 
            let bonus = Math.floor(points / 2); 
            teams.forEach(t => { 
                if(t !== team) teamScores[t] += bonus; 
            });
...
function closeQuestionPopup() { 
    ...
    // VIKTIGT: Nollställ eventet i databasen när frågan stängs
    updateDoc(doc(db, "rooms", roomId), { event: null }); 
    
    if (Object.keys(viewedQuestions).length === totalQuestions) {
...
function showEndScreen() {
    ...
    resultsDiv.innerHTML = `<h2>Vinnare: ${sortedTeams[0][0]} med ${sortedTeams[0][1]} poäng!</h2>`;
